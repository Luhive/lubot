-- Migration: Lubot initial schema
-- Purpose: Create groups, broadcasts, and knowledge_chunks tables with seed data
-- Affected tables: groups, broadcasts, knowledge_chunks
-- Notes: RLS disabled for hackathon; pgvector extension required; seed data pre-loads
--        22 knowledge chunks for the onboarding agent demo.
...
-- Grant access to anon and authenticated roles (no auth wall during hackathon)
grant all on groups to anon, authenticated;
grant all on broadcasts to anon, authenticated;
